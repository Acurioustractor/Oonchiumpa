document.addEventListener('DOMContentLoaded', function() {
    ...
    function selectQuarter(quarter) {
        ...
        quarterDescriptionElement.innerHTML = `
            ...
            <p class="description-text">This activity aligns with our strategic goal to ${getStrategicGoal(quarter.id)} and supports NIAA's priority for ${getNIAAPriority(quarter.id)}.</p>
        `;
    ...
    // Get strategic goal based on quarter ID
    function getStrategicGoal(quarterId) {
        const year = parseInt(quarterId.charAt(1));
        
        if (year === 1) {
            return "strengthen organizational capacity and establish key partnerships";
        } else if (year === 2) {
            return "expand service delivery and develop new program offerings";
        } else {
            return "ensure long-term sustainability and systemic integration";
        }
    }

    // Get NIAA priority based on quarter ID
    function getNIAAPriority(quarterId) {
        const year = parseInt(quarterId.charAt(1));
        const quarter = parseInt(quarterId.charAt(3));
        
        if (year === 1 && (quarter === 1 || quarter === 2)) {
            return "community safety and crime prevention";
        } else if (year === 1 && (quarter === 3 || quarter === 4)) {
            return "youth justice diversion and rehabilitation";
        } else if (year === 2 && (quarter === 1 || quarter === 2)) {
            return "youth justice diversion and rehabilitation";
        } else if (year === 2 && (quarter === 3 || quarter === 4)) {
            return "building community resilience and leadership";
        } else {
            return "building community resilience and leadership";
        }
    }
