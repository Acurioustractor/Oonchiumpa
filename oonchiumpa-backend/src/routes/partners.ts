...
const mockPartners = [
  {
    ...
    category: "Government",
    description:
      "Key partner supporting youth health and wellbeing programs across remote Northern Territory communities.",
  ...
  },
  {
    ...
    description:
      "National organization promoting reconciliation between Aboriginal and Torres Strait Islander peoples and non-Indigenous Australians.",
  ...
  },
  {
    ...
    description:
      "Supporting Indigenous youth education and leadership development programs through significant funding and mentorship opportunities.",
  ...
  },
  {
    ...
    description:
      "Research collaboration and student placement partnerships supporting evidence-based program development.",
  ...
  },
  {
    ...
    category: "NGO",
    description:
      "Partnership in emergency response and community resilience programs, particularly in remote Indigenous communities.",
  ...
  },
  {
    ...
    category: "Corporate",
    description:
      "Supporting Indigenous arts and cultural expression programs, providing both funding and exhibition opportunities.",
  ...
  },
  {
    ...
    description:
      "Land-based partnerships supporting traditional knowledge sharing and country management programs.",
    website:
      "https://www.niaa.gov.au/indigenous-affairs/environment/indigenous-protected-areas",
    logo: null,
    isActive: true,
    partnershipLevel: "CORE_PARTNER",
  ...
  },
  {
    ...
    description:
      "Pro-bono consulting services for organizational development, impact measurement, and strategic planning.",
    website:
      "https://home.kpmg/au/en/home/services/advisory/indigenous-consulting.html",
    logo: null,
    isActive: true,
    partnershipLevel: "COLLABORATOR",
    monetaryValue: 0,
  ...
  try {
    ...
    if (active !== undefined) {
      ...
      filteredPartners = filteredPartners.filter(
        (partner) => partner.isActive === isActive,
      );
    }
    ...
    if (level && typeof level === "string") {
      filteredPartners = filteredPartners.filter(
        (partner) => partner.partnershipLevel === level,
      );
    }
    ...
    const stats = {
      ...
      totalValue: filteredPartners.reduce(
        (sum, p) => sum + (p.monetaryValue || 0) + (p.inKindValue || 0),
        0,
      ),
      partnershipLevels: {
        prospect: filteredPartners.filter(
          (p) => p.partnershipLevel === "PROSPECT",
        ).length,
        collaborator: filteredPartners.filter(
          (p) => p.partnershipLevel === "COLLABORATOR",
        ).length,
        strategic: filteredPartners.filter(
          (p) => p.partnershipLevel === "STRATEGIC_PARTNER",
        ).length,
        core: filteredPartners.filter(
          (p) => p.partnershipLevel === "CORE_PARTNER",
        ).length,
        legacy: filteredPartners.filter(
          (p) => p.partnershipLevel === "LEGACY_PARTNER",
        ).length,
      },
      totalCommunities: [
...
router.get("/stats/summary", async (req: Request, res: Response) => {
  try {
    ...
    const stats = {
      ...
      totalMonetaryValue: activePartners.reduce(
        (sum, p) => sum + (p.monetaryValue || 0),
        0,
      ),
      totalInKindValue: activePartners.reduce(
        (sum, p) => sum + (p.inKindValue || 0),
        0,
      ),
      totalValue: activePartners.reduce(
        (sum, p) => sum + (p.monetaryValue || 0) + (p.inKindValue || 0),
        0,
      ),
      partnershipLevels: {
        prospect: activePartners.filter(
          (p) => p.partnershipLevel === "PROSPECT",
        ).length,
        collaborator: activePartners.filter(
          (p) => p.partnershipLevel === "COLLABORATOR",
        ).length,
        strategic: activePartners.filter(
          (p) => p.partnershipLevel === "STRATEGIC_PARTNER",
        ).length,
        core: activePartners.filter(
          (p) => p.partnershipLevel === "CORE_PARTNER",
        ).length,
        legacy: activePartners.filter(
          (p) => p.partnershipLevel === "LEGACY_PARTNER",
        ).length,
      },
      partnerTypes: {
        ...
        educational: activePartners.filter((p) => p.type === "EDUCATIONAL")
          .length,
        community: activePartners.filter(
          (p) => p.type === "COMMUNITY_ORGANIZATION",
        ).length,
      },
      totalCommunities: [
