...
import { culturalValidationMiddleware } from '../middleware/culturalValidation';
...
// Create new story
router.post('/', culturalValidationMiddleware, async (req, res, next) => {
  try {
    const {
      title,
      subtitle,
...
// Update story
router.put('/:id', culturalValidationMiddleware, async (req, res, next) => {
  try {
    const { id } = req.params;
