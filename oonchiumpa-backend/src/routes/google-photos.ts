...
import { googlePhotos } from '../services/googlePhotosIntegration';
...
router.post('/auth/url', async (req, res) => {
  try {
    ...
    logger.error('Error generating Google Photos auth URL:', error);
    res.status(500).json({
      success: false,
...
router.get('/auth/callback', async (req, res) => {
  try {
    ...
    const tokens = await googlePhotos.setCredentials(code);
    ...
    logger.error('Error handling Google Photos OAuth callback:', error);
    res.status(500).json({
      success: false,
...
router.get('/albums', async (req, res) => {
  try {
    const albums = await googlePhotos.getAlbums();
    ...
    logger.error('Error fetching Google Photos albums:', error);
    res.status(500).json({
      success: false,
...
router.get('/:albumId?', async (req, res) => {
  try {
    ...
    const photos = await googlePhotos.getPhotos(
      albumId, 
    ...
    logger.error('Error fetching Google Photos:', error);
    res.status(500).json({
      success: false,
...
router.post('/search/date', async (req, res) => {
  try {
    ...
    const photos = await googlePhotos.searchPhotosByDateRange(
      new Date(startDate),
      new Date(endDate)
    );
    ...
    logger.error('Error searching photos by date:', error);
    res.status(500).json({
      success: false,
...
router.post('/import', async (req, res) => {
  try {
    ...
    for (const photoId of photoIds) {
      try {
        const photoInfo = await googlePhotos.getPhotoSharingInfo(photoId);
        photos.push(photoInfo);
      } catch (error) {
        logger.warn(`Failed to get photo info for ${photoId}:`, error);
      }
    }
    ...
    // Import photos
    const result = await googlePhotos.importPhotos(photos, {
      programId,
      outcomeId,
      storyId,
      uploadedById: userId,
    ...
    logger.error('Error importing Google Photos:', error);
    res.status(500).json({
      success: false,
...
router.post('/import/album/:albumId', async (req, res) => {
  try {
    ...
    const progressHandler = io ? (progress: any) => {
    ...
    logger.info(`🚀 Starting batch import from album ${albumId} for user ${userId}`);

    const result = await googlePhotos.batchImportFromAlbum(albumId, {
      uploadedById: userId,
      programId,
      outcomeId,
    ...
    logger.error('Error in batch album import:', error);
    res.status(500).json({
      success: false,
...
router.get('/imports/history', async (req, res) => {
  try {
    ...
    const imports = await prisma.mediaItem.findMany({
      where: {
        uploadedById: userId,
        tags: {
          has: 'google-photos'
