...
exports.debugEnv = exports.env = exports.config = void 0;
...
class ConfigManager {
    ...
    findEnvFiles() {
        const possiblePaths = [
            ...
            path_1.default.resolve(process.env.HOME || "~", ".oonchiumpa.env"),
        ];
        this.envPaths = possiblePaths.filter((envPath) => {
            try {
                return fs_1.default.existsSync(envPath);
            }
            catch (error) {
                return false;
            }
    ...
    createDefaultEnvFile() {
        ...
        try {
            fs_1.default.writeFileSync(defaultEnvPath, defaultEnvContent);
...
`;
    ...
    validateAndParseConfig() {
        const config = {
            ...
            DEFAULT_SENSITIVITY_LEVEL: process.env.DEFAULT_SENSITIVITY_LEVEL || "community",
            // Frontend
     ...
     */
    logAIProviderStatus(config) {
        console.log("\n🤖 AI Provider Status:");
        const providers = [
            {
                name: "OpenAI",
                key: "OPENAI_API_KEY",
                available: !!config.OPENAI_API_KEY && !config.OPENAI_API_KEY.includes("your-"),
            },
            {
                name: "Anthropic",
    ...
    isAIProviderAvailable(provider) {
        switch (provider) {
            ...
            case "google_vision":
                return (!!this.config.GOOGLE_CLOUD_VISION_API_KEY &&
                    !this.config.GOOGLE_CLOUD_VISION_API_KEY.includes("your-"));
            case "azure_vision":
                return (!!this.config.AZURE_COMPUTER_VISION_KEY &&
...
exports.config = configManager.getConfig();
exports.env = configManager;
...
const debugEnv = () => {
    ...
    console.log("Config loaded:", !!exports.config);
};
