...
const storage = multer_1.default.diskStorage({
    destination: async (req, file, cb) => {
        const uploadPath = path_1.default.join(process.cwd(), 'uploads', 'bulk');
...
router.post('/upload', upload.array('files', 50), async (req, res, next) => {
    try {
        const { category = 'general', tags = [], authorUserId, processImmediately = true, culturalContext = {} } = req.body;
...
router.post('/process/:documentId', async (req, res, next) => {
    try {
        ...
        const { providers = ['openai', 'anthropic'], extractTypes = ['stories', 'outcomes', 'insights'], culturalReviewRequired = true } = req.body;
        ...
        if (document.mimeType.startsWith('text/') || document.mimeType.includes('pdf') || document.mimeType.includes('word')) {
            ...
            aiResults = await aiOrchestrator_1.aiOrchestrator.processInterview(content, {
            ...
            // Image processing
            aiResults = await aiOrchestrator_1.aiOrchestrator.analyzeMedia(document.filePath, 'image');
        }
        else if (document.mimeType.startsWith('video/')) {
            // Video processing
            aiResults = await aiOrchestrator_1.aiOrchestrator.analyzeMedia(document.filePath, 'video');
        }
...
router.post('/import-platform-docs', async (req, res, next) => {
    try {
        ...
        // Import interviews
        const interviewsPath = path_1.default.join(platformPath, 'Interviews');
        const interviews = await importFromDirectory(interviewsPath, 'interview', authorUserId, processImmediately);
        // Import reports  
        const reportsPath = path_1.default.join(platformPath, 'Reports');
...
async function createContentFromAI(aiResults, document, culturalReviewRequired) {
    ...
    for (const result of aiResults) {
        if (result.type === 'story' && result.content.items) {
            for (const storyData of result.content.items.filter((item) => item.type === 'story')) {
                ...
                if (result.culturalSensitivity?.requiresReview) {
                    await (0, culturalValidation_1.requestCulturalConsultation)('story', story.id, `Cultural review required for story extracted from ${document.filename}`, document.uploadedById, result.culturalSensitivity.sensitivityLevel === 'SACRED' ? 'URGENT' : 'HIGH');
                    createdContent.consultationsRequired++;
                }
            }
        }
        if (result.type === 'outcome' && result.content.items) {
            for (const outcomeData of result.content.items.filter((item) => item.type === 'outcome')) {
...
function mapSensitivityLevel(level) {
...
async function processSingleDocument(documentId, processors, culturalReviewRequired) {
    // Implementation for single document processing
    return { status: 'processed' };
}
async function importFromDirectory(dirPath, category, authorUserId, processImmediately) {
    const results = [];
    try {
