...
const contentOrchestrator_1 = require("../services/contentOrchestrator");
const logger_1 = require("../utils/logger");
const auth_1 = require("../middleware/auth");
const router = express_1.default.Router();
const contentOrchestrator = new contentOrchestrator_1.ContentOrchestrator();
/**
 ...
 */
router.post('/team-profile', auth_1.authenticateToken, auth_1.requireAIAccess, async (req, res) => {
    try {
        const { memberName, interviewFiles } = req.body;
        logger_1.logger.info(`🎭 Generating team profile for: ${memberName}`);
        ...
        logger_1.logger.info(`✅ Team profile generated: ${profile.title}`);
        res.json({
            success: true,
            profile: profile
        });
        ...
        logger_1.logger.error('Team profile generation error:', error);
        res.status(500).json({
            success: false,
 ...
 * REQUIRES: Oonchiumpa staff or admin access
 */
router.post('/historical-piece', auth_1.authenticateToken, auth_1.requireAIAccess, async (req, res) => {
    try {
        const { topic, sourceDocuments } = req.body;
        logger_1.logger.info(`📜 Generating historical piece: ${topic}`);
        ...
        logger_1.logger.info(`✅ Historical piece generated: ${historicalContent.title}`);
        res.json({
            success: true,
        ...
        logger_1.logger.error('Historical piece generation error:', error);
        res.status(500).json({
            success: false,
 ...
 * REQUIRES: Oonchiumpa staff or admin access
 */
router.post('/transformation-story', auth_1.authenticateToken, auth_1.requireAIAccess, async (req, res) => {
    try {
        const { focusArea, sourceDocuments } = req.body;
        logger_1.logger.info(`🌟 Generating transformation story: ${focusArea}`);
        ...
        logger_1.logger.info(`✅ Transformation story generated: ${story.title}`);
        res.json({
            success: true,
            story: story
        });
        ...
        logger_1.logger.error('Transformation story generation error:', error);
        res.status(500).json({
            success: false,
 ...
 * REQUIRES: Oonchiumpa staff or admin access
 */
router.post('/blog-post', auth_1.authenticateToken, auth_1.requireAIAccess, async (req, res) => {
    try {
        const { theme, recentDocuments } = req.body;
        logger_1.logger.info(`📝 Generating blog post: ${theme}`);
        ...
        logger_1.logger.info(`✅ Blog post generated: ${blogPost.title}`);
        res.json({
            success: true,
            blogPost: blogPost
        });
        ...
        logger_1.logger.error('Blog post generation error:', error);
        res.status(500).json({
            success: false,
 ...
 */
router.post('/build-library', auth_1.authenticateToken, (0, auth_1.requirePermission)('ai.configure'), async (req, res) => {
    try {
        logger_1.logger.info('🏗️ Building comprehensive content library from all documents...');
        ...
        logger_1.logger.info(`✅ Content library built: ${library.teamProfiles.length + library.blogPosts.length + library.caseStudies.length + library.historicalPieces.length} pieces generated`);
        res.json({
            success: true,
            library: {
        ...
        logger_1.logger.error('Content library generation error:', error);
        res.status(500).json({
            success: false,
 ...
 * REQUIRES: Oonchiumpa staff or admin access
 */
router.post('/custom', auth_1.authenticateToken, auth_1.requireAIAccess, async (req, res) => {
    try {
        const request = req.body;
        logger_1.logger.info(`🎯 Generating custom ${request.type}: ${request.focusArea}`);
        ...
        logger_1.logger.info(`✅ Custom content generated: ${content.title}`);
        res.json({
            success: true,
            content: content
        });
        ...
        logger_1.logger.error('Custom content generation error:', error);
        res.status(500).json({
            success: false,
 ...
 */
router.get('/demo', auth_1.optionalAuth, async (req, res) => {
    try {
        logger_1.logger.info('🚀 Generating demo content showcase...');
        ...
        logger_1.logger.info('✅ Demo content generated successfully');
        res.json({
            success: true,
        ...
        logger_1.logger.error('Demo content generation error:', error);
        res.status(500).json({
            success: false,
