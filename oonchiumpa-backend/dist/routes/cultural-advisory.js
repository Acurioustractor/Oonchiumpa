...
router.post('/consultations', async (req, res, next) => {
    try {
        const { contentType, contentId, purpose, advisorId, priority = 'MEDIUM', requestedById } = req.body;
...
router.get('/consultations', async (req, res, next) => {
    try {
        const { status, priority, advisorId, contentType, limit = '20', offset = '0' } = req.query;
        const where = {};
        if (status)
            where.status = status;
        if (priority)
...
router.put('/consultations/:id', async (req, res, next) => {
    try {
        const { id } = req.params;
        const { status, notes, recommendations, culturalSensitivityRating, approvalStatus, followUpRequired, scheduledAt, completedAt } = req.body;
        const updateData = {};
        if (status !== undefined)
        ...
        if (culturalSensitivityRating !== undefined)
            updateData.culturalSensitivityRating = culturalSensitivityRating;
...
router.post('/protocols', async (req, res, next) => {
    try {
        const { storyTitle, storyType, traditionalOwners, geographicOrigin, seasonalRestrictions = [], genderRestrictions, ageRestrictions, ceremonialContext, sharingPermissions, consultationRequired = true, advisorId, validatedAt, expiresAt } = req.body;
        // Validate required fields
...
router.get('/protocols', async (req, res, next) => {
    try {
        ...
        const where = {};
        if (storyTitle)
            where.storyTitle = { contains: storyTitle, mode: 'insensitive' };
        if (storyType)
...
//# sourceMappingURL=cultural-advisory.js.map