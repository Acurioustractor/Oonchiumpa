...
router.post('/', async (req, res) => {
    try {
        ...
        logger_1.logger.info(`🔍 Processing document: ${filename || 'untitled'}`);
        // Process with AI
        const results = await aiOrchestrator_1.aiOrchestrator.processInterview(content, {
        ...
        logger_1.logger.info(`✅ AI processing completed: ${results.length} items extracted`);
        ...
        logger_1.logger.error('Error processing document:', error);
        res.status(500).json({
            success: false,
...
router.post('/save', async (req, res) => {
    try {
        ...
        logger_1.logger.info(`📄 Processing and saving document: ${filename || 'untitled'}`);
        // Process with AI
        const results = await aiOrchestrator_1.aiOrchestrator.processInterview(content, {
            filename: filename || 'test-document',
            type,
        ...
        for (const story of storyResults) {
            if (story.confidence > 0.7) { // Only save high-confidence stories
                const savedStory = await index_1.prisma.story.create({
                    data: {
        ...
        for (const outcome of outcomeResults) {
            if (outcome.confidence > 0.7) { // Only save high-confidence outcomes
                const savedOutcome = await index_1.prisma.outcome.create({
                    data: {
        ...
        logger_1.logger.info(`✅ Saved ${savedStories.length} stories and ${savedOutcomes.length} outcomes to database`);
        res.json({
            success: true,
        ...
        logger_1.logger.error('Error processing and saving document:', error);
        res.status(500).json({
            success: false,
